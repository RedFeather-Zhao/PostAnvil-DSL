...
仓库的 `.github/workflows/native.yml` 会在 push、Pull Request 或手动触发时运行。它为
Windows、Ubuntu 和 macOS 分别构建 Debug、Release 静态库与动态库，执行原生测试，安装
CMake Package，并使用 `tests/package_consumer` 从安装目录重新执行 `find_package`、链接和
运行验证。通过后会上传六个独立 artifact：

```text
PostAnvil-windows-X64-Debug
PostAnvil-windows-X64-Release
PostAnvil-ubuntu-X64-Debug
PostAnvil-ubuntu-X64-Release
PostAnvil-macos-ARM64-Debug
PostAnvil-macos-ARM64-Release
```

artifact 中的 ZIP 包包含对应平台的头文件、静态库、动态库和
...
```text
PostAnvil-android-arm64-v8a
PostAnvil-android-armeabi-v7a
PostAnvil-android-all-abis
```
