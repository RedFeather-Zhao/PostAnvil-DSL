...
TEST_CLASS(ValueAndBuiltinTests)
{
public:
	TEST_METHOD(StringConcatenationAndGlobalReference)
	{
	...
	TEST_METHOD(NormalizedProperties)
	{
	...
	TEST_METHOD(CustomClassProperties)
	{
	...
	TEST_METHOD(StringVariableAsClassExpression)
	{
		evaluate_and_expect_counts(R"(
			STR target = "animal"
			RULE FILTER target:
	...
	TEST_METHOD(GlobalNumericVariables)
	{
		evaluate_and_expect_counts(R"(
			NUM min_conf = 0.6
			NUM min_w = 15
	...
	TEST_METHOD(GlobalBooleanVariable)
	{
		evaluate_and_expect_counts(R"(
			BOOL debug = FALSE
			RULE FILTER "global":
	...
	TEST_METHOD(MathAndSpatialBuiltins)
	{
		auto output = evaluate(R"(
	...
	TEST_METHOD(InstanceIdentityAndLookup)
	{
		auto output = evaluate_and_expect_counts(R"(
	...
	TEST_METHOD(WritableBuiltinInstanceProperties)
	{
		auto output = evaluate(R"(
			RULE ATTR "box":
				self.x1 = 3
	...
	TEST_METHOD(MissingClassesBehaveAsEmptyCollections)
	{
		auto output = evaluate_and_expect_counts(R"(
