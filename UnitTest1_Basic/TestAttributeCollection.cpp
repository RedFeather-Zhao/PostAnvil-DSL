...
TEST_CLASS(TestAttributeAndCollection)
{
public:
	TEST_METHOD(InstanceAttributesAndTypedGlobals)
	{
	...
	TEST_METHOD(AttributeThenFilter)
	{
	...
	TEST_METHOD(KeywordsAndNamesAreCaseInsensitive)
	{
		evaluate_and_expect_counts(R"(
			rule attr "X":
				"X".test = 1
	...
	TEST_METHOD(GlobalAttributeRule)
	{
		auto output = evaluate(R"(
			RULE ATTR "global":
	...
	TEST_METHOD(GroupCreatesDerivedClass)
	{
		evaluate_and_expect_counts(R"(
	...
	TEST_METHOD(AppendCreatesDerivedClass)
	{
		evaluate_and_expect_counts(R"(
	...
	TEST_METHOD(ClassCountProperty)
	{
		evaluate_and_expect_counts(R"(
