...
TEST_CLASS(BasicFilterRuleTests)
{
public:
	TEST_METHOD(FilterTest_1_Conf)
	{
	...
	TEST_METHOD(FilterTest_2_Area_W_H)
	{
		evaluate_and_expect_counts(R"(
				RULE FILTER "Global":
					self.w > 10
	...
	TEST_METHOD(FilterTest_4_Add_Minus)
	{
		evaluate_and_expect_counts(R"(
			RULE FILTER "d":
				self.x1 + self.w > 100
	...
	TEST_METHOD(FilterTest_5_Mul_Dev)
	{
		evaluate_and_expect_counts(R"(
			RULE FILTER "e":
				self.w * self.h > 300
	...
	TEST_METHOD(FilterTest_6_Or_And)
	{
		evaluate_and_expect_counts(R"(
			RULE FILTER "f":
	...
	TEST_METHOD(FilterTest_7_Image)
	{
		evaluate_and_expect_counts(R"(
			RULE FILTER "h":
	...
	TEST_METHOD(FilterTest_8_DerivedProperties)
	{
		evaluate_and_expect_counts(R"(
			RULE FILTER "aa":
				self.cx > 50
	...
	TEST_METHOD(FilterTest_8_MultiClass)
	{
		evaluate_and_expect_counts(R"(
			RULE FILTER "global":
				self.conf > 0.5
