...
std::vector<TestCase> get_import_export_tests() {
	...
	tests.emplace_back("IMPORT 导入宿主变量",
		R"(
	...
	tests.emplace_back("IMPORT AS 别名 (host_conf AS conf_threshold)",
		R"(
	...
	tests.emplace_back("EXPORT 导出值精确验证",
		R"(
			RULE ATTR "person":
  ...
  return false;
	...
	tests.emplace_back("INST 类型 IMPORT / EXPORT",
		R"(
			IMPORT INST anchor
